...
- **Regla de Oro PWA**: Implementar arquitectura **App Shell**. La interfaz básica (header, sidebar, loading state) siempre carga instántaneamente desde el caché.
- **Integración PWA Obligatoria**:
    - `manifest.json`: Definir nombre, iconos (192x192 y 512x512), colores y `display: standalone`.
    - `service-worker.js`: Estrategia **Stale-while-revalidate** para asegurar que el contenido sea offline-first pero se actualice en segundo plano.
    - Registro del Service Worker en el `index.html`.
