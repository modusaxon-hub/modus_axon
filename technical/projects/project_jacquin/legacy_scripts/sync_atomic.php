...
try {
    ...
    foreach($dbCourses as $c) {
        if (!in_array($c['course_name'], $titlesToKeep) || in_array($c['course_name'], $killList)) {
            $cid = $c['id_course'];
            echo "Purging relations for id: $cid (" . $c['course_name'] . ")\n";
            
            // Delete in order using direct IDs
            ...
            // Sched_teachers needs to join with schedules to get IDs
            $pdo->exec("DELETE st FROM schedule_teachers st INNER JOIN schedules s ON st.id_schedule = s.id_schedule WHERE s.id_course = $cid");
